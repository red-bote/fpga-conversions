#!/bin/bash
# Create the initial Vivado Basys3 project for the Kick port in the
...
# the project sources tree is basys3/kick_basys3.srcs/.
#
...
CONSTRS_IMPORT="$PROJ_DIR/kick_basys3.srcs/constrs_1/imports/digilent-xdc-master"

if [ ! -d "$SRC_DIR" ]; then
    echo "error: source tree not found: $SRC_DIR" >&2
    echo "Run contrib/tools/setup_kick.sh first." >&2
    exit 1
fi
...
step "2/3 Copying kick_basys3.xpr"
cp -f "$CONTRIB/vivado/kick_basys3.xpr" "$PROJ_DIR/kick_basys3.xpr"
