...
- PS/2 keyboard on **JB** (JB1 = `ps2_data`, JB3 = `ps2_clk`) via the unchanged `io_ps2_keyboard` scancode decoder + `kbd_joystick` scancode→button mapper (note: the specific F-key bindings differ per machine).
...
- Audio on **PmodAMP2** (JC): the core's digital `audio_out` feeds a PWM accumulator (same technique as the DE10-Lite) whose MSB drives `AIN`; `sw(15)`/`sw(14)` control AMP gain/shutdown.
