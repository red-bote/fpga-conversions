#!/bin/bash
# Linux rom-prep for the Pooyan Basys3 port.
#
# 1. Compile make_vhdl_prom from the extracted Dar source tree on the host (gcc).
# 2. Convert make_pooyan_proms.bat -> make_pooyan_proms.sh.
# 3. Unzip the romset (~/roms/pooyan.zip) into tools/pooyan_unzip/.
# 4. Run make_pooyan_proms.sh to generate the PROM VHDL.
#
# Requires the pristine Dar source tree (vhdl_pooyan_rev_0_2_2020_04_26/)
...
if [ ! -f "$TOOLS_SRC/make_vhdl_prom.c" ]; then
    ...
    echo "Run setup_pooyan.sh first." >&2
    exit 1
fi
