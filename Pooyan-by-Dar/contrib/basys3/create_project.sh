...
regenerate_wrapper() {
...

  {
    ...
    echo " vga_r   <= r6o(${r6hi} downto ${r6lo});"
    echo " vga_g <= g6o(${r6hi} downto ${r6lo});"
    echo " vga_b  <= b6o(${r6hi} downto ${r6lo});"
    echo ""
...
# Regenerate the XDC from the Digilent Basys-3-Master.xdc template (committed
# in contrib/basys3/): uncomment the pins used by the README IO mapping and
# rename each get_ports target to the wrapper port; every unused xc7 pin stays
# commented. Config options and the header pass through unchanged.
generate_xdc() {
  local master="$CONTRIB_DIR/Basys-3-Master.xdc"
  [[ -f "$master" ]] || die "missing Digilent master template $master (commit it; origin + SHA-256 recorded in the README)"
  mkdir -p "$(dirname "$XDC")"
  local -A pin_target
  local i idx port width pins p t
  for i in "${!io_port[@]}"; do
    port="${io_port[$i]}"; width="${io_width[$i]}"
    IFS=';' read -r -a pins <<< "${io_pins[$i]}"
    for idx in "${!pins[@]}"; do
      p="${pins[$idx]}"
      if (( width > 1 )); then t="$port[$idx]"; else t="$port"; fi
      pin_target["$p"]="$t"
    done
  done
  local period
  period="$(awk "BEGIN{printf \"%.2f\", 1000 / ${fact[clk_wiz.input]:-100.000}}")"
  {
    ...
    echo "# from the Digilent Basys-3-Master.xdc template (contrib/basys3/,"
    echo "# https://github.com/Digilent/digilent-xdc/blob/master/Basys-3-Master.xdc)."
    echo "# Uncommented lines are the pins used by the IO mapping in"
    echo "# Pooyan-by-Dar/README.md (source of truth); unused pins stay commented."
    echo "#"
    local line target
    while IFS= read -r line; do
      if [[ "$line" =~ PACKAGE_PIN[[:space:]]+([A-Za-z0-9]+) ]]; then
        target="${pin_target[${BASH_REMATCH[1]}]:-}"
        if [[ -n "$target" ]]; then
          line="${line#\#}"
          line="$(printf '%s\n' "$line" | sed -E "s/\[get_ports \{[^}]*\}\]/[get_ports {$target}]/; s/\[get_ports [A-Za-z0-9_]+]/[get_ports $target]/")"
          if [[ "$target" == JA* ]] && [[ "${fact[ja.pullup]:-0}" == "1" ]]; then
            line="$(printf '%s\n' "$line" | sed -E 's/IOSTANDARD LVCMOS33[[:space:]]*\}/IOSTANDARD LVCMOS33 PULLUP true }/')"
          fi
          printf '%s\n' "$line"
        else
          printf '%s\n' "$line"
        fi
      elif [[ "$line" =~ ^#?create_clock ]]; then
        if [[ -n "${pin_target[W5]:-}" ]]; then
          # -add keeps this alongside clk_wiz_0's in-context create_clock instead
          # of overriding it (Constraints 18-1056 / scoped-clock DRC otherwise fires).
  ...
  echo "port    pooyan_basys3.xdc -> constrs_1/imports/digilent-xdc-master/ (generated from Basys-3-Master.xdc + README IO mapping)"
}
