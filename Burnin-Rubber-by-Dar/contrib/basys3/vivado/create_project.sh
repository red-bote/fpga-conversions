#!/bin/bash
# Create the initial Vivado Basys3 project for the Burnin' Rubber port in the
...
CONSTRS_IMPORT="$PROJ_DIR/burnin_rubber_basys3.srcs/constrs_1/imports/digilent-xdc-master"
SOURCES_IMPORT="$PROJ_DIR/burnin_rubber_basys3.srcs/sources_1/imports/mist"

if [ ! -d "$SRC_DIR" ]; then
    echo "error: source tree not found: $SRC_DIR" >&2
    echo "Run contrib/tools/setup_burnin_rubber.sh first." >&2
    exit 1
fi
...
ls -l "$PROJ_DIR/burnin_rubber_basys3.xpr"
