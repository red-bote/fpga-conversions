#!/bin/bash
# Drive Vivado to synthesise and build the Solar Fox Basys3 bitstream.
#
...
# create_prj`, `make clk_wiz` and `make patch` first (all covered by `make
# synth`). The .xpr's impl_1 run has GenFullBitstream=true, so it emits
#   vhdl_solar_fox_rev_0_1_2019_11_22/basys3/solar_fox_basys3.runs/impl_1/solar_fox_basys3.bit
#
...
XPR="$ROOT/vhdl_solar_fox_rev_0_1_2019_11_22/basys3/solar_fox_basys3.xpr"
...
if [ "$MODE" = "synth" ]; then
    ...
    BIT="$ROOT/vhdl_solar_fox_rev_0_1_2019_11_22/basys3/solar_fox_basys3.runs/impl_1/solar_fox_basys3.bit"
    if [ -f "$BIT" ]; then
        echo "Bitstream: $BIT"
    else
