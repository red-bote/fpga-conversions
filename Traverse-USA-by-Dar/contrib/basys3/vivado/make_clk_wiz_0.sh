#!/bin/bash
# Generate the clk_wiz_0 MMCM IP (100 MHz -> 36.86 MHz core + 7.16 MHz) for
# the Basys3 port and place its Verilog wrappers where traverse_usa_basys3.xpr
# expects them.
#
...
# divides it by 2 in fabric to ~3.58 MHz (same approach as the Defender port;
# the core's 3.58 MHz input and the PWM clock tolerate the small error).
#
...
# The IP is generated here in a throwaway Vivado project named mmcm_3686 and
...
XPR_DIR="$(cd "$(dirname "$0")/../../../vhdl_traverse_usa_rev_0_0_2019_03_16/basys3" && pwd)"
CLK_WIZ_IMPORT_DIR="$XPR_DIR/traverse_usa_basys3.srcs/sources_1/imports/clk_wiz_0"
...
cat > "$TCL" <<EOF
create_project mmcm_3686 "$WORK" -part $PART -force
...
set_property -dict [list \
    ...
    CONFIG.CLKOUT1_REQUESTED_OUT_FREQ {36.86} \
