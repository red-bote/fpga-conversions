#!/bin/bash
# Create the initial Vivado Basys3 project for the Traverse USA port in the
...
# the project sources tree is basys3/traverse_usa_basys3.srcs/.
#
# 1. Create the project dirs.
# 2. Copy the .xpr (already authored with its scandoubler File Path pointing
#    at sources_1/imports/mist/scandoubler.v).
# 3. Copy Basys-3-Master.xdc into constrs_1/imports/digilent-xdc-master/.
# 4. Copy scandoubler.v into sources_1/imports/mist/ (the core is native
#    15 kHz composite sync, so the external MiST scandoubler provides 31 kHz
#    VGA).
#
...
CONSTRS_IMPORT="$PROJ_DIR/traverse_usa_basys3.srcs/constrs_1/imports/digilent-xdc-master"
SOURCES_IMPORT="$PROJ_DIR/traverse_usa_basys3.srcs/sources_1/imports/mist"

if [ ! -d "$SRC_DIR" ]; then
    echo "error: source tree not found: $SRC_DIR" >&2
    echo "Run contrib/tools/setup_traverse_usa.sh first." >&2
    exit 1
fi
...
step "2/4 Copying traverse_usa_basys3.xpr"
cp -f "$CONTRIB/vivado/traverse_usa_basys3.xpr" "$PROJ_DIR/traverse_usa_basys3.xpr"
...
ls -l "$PROJ_DIR/traverse_usa_basys3.xpr"
