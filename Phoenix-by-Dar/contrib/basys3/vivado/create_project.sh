#!/bin/bash
# Create the initial Vivado Basys3 project for the Phoenix port in the
# extracted Dar source tree, and copy the tracked port assets into place.
#
# Layout is non-nested (like Defender/BurgerTime and Galaga): the .xpr lives
# directly in basys3/ and the project sources tree is basys3/phoenix_basys3.srcs/.
#
...
CONSTRS_IMPORT="$PROJ_DIR/phoenix_basys3.srcs/constrs_1/imports/digilent-xdc-master"
SOURCES_IMPORT="$PROJ_DIR/phoenix_basys3.srcs/sources_1/imports/mist"

if [ ! -d "$SRC_DIR" ]; then
    echo "error: source tree not found: $SRC_DIR" >&2
    echo "Run contrib/tools/setup_phoenix.sh first." >&2
    exit 1
fi
...
step "2/4 Copying phoenix_basys3.xpr"
cp -f "$CONTRIB/vivado/phoenix_basys3.xpr" "$PROJ_DIR/phoenix_basys3.xpr"
...
ls -l "$PROJ_DIR/phoenix_basys3.xpr"
