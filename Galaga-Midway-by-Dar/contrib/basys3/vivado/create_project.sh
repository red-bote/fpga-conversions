#!/bin/bash
# Create the initial Vivado Basys3 project for the Galaga port in the
...
# the project sources tree is basys3/galaga_basys3.srcs/.
#
...
CONSTRS_IMPORT="$PROJ_DIR/galaga_basys3.srcs/constrs_1/imports/digilent-xdc-master"
SOURCES_IMPORT="$PROJ_DIR/galaga_basys3.srcs/sources_1/imports/mist"

if [ ! -d "$SRC_DIR" ]; then
    echo "error: source tree not found: $SRC_DIR" >&2
    echo "Run contrib/tools/setup_galaga.sh first." >&2
    exit 1
fi
...
step "2/4 Copying galaga_basys3.xpr (re-pointing scandoubler)"
cp -f "$CONTRIB/vivado/galaga_basys3.xpr" "$PROJ_DIR/galaga_basys3.xpr"
sed -i 's|\$PPRDIR/../../../Arcade_Galaga/mist/scandoubler.v|\$PSRCDIR/sources_1/imports/mist/scandoubler.v|' \
    "$PROJ_DIR/galaga_basys3.xpr"
...
ls -l "$PROJ_DIR/galaga_basys3.xpr"
