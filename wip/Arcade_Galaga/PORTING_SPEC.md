...
- **Display-mode switch** via **F8 key** (`fn_toggle(7)`, same convention as the Midway port):
  - `fn_toggle(7)=0` (default): 31 kHz VGA — scan-doubled 6-bit RGB narrowed to 4 bits
    (`*_o(5 downto 2)`), `hsync_o`/`vsync_o`.
  - `fn_toggle(7)=1`: 15 kHz TV — native core RGB padded to 4 bits (`r&'0'`, `g&'0'`, `b&"00"`,
...
### 7.3 Dip switches (sw12..sw0) — active-high, inverted polarity

The core's `dip_switch_a`/`dip_switch_b` inputs (added to `galaga.vhd` entity)
are wired to Basys3 switches with **inverted polarity**: all switches DOWN =
normal play defaults.
...
| Switch | Dip bit(s) | Function | UP (1) | DOWN (0) |
|---|---|---|---|---|
| sw(12) | A7 | Cabinet | Upright | Cocktail |
| sw(11) | A5 | Test mode | Off (normal) | On |
| sw(10) | A4 | Freeze | Off (normal) | On |
| sw(9) | A3 | Demo sound | Off | On (normal) |
| sw(8:7) | A1:0 | Difficulty | 00=easiest .. 11=normal | |
| sw(6:5) | B7:6 | Lives | 00=2, 01=3, 10=2, 11=5 | |
| sw(4:2) | B5:3 | Bonus | 000=none .. 101=10k .. 111=40k | |
| sw(1:0) | — | Reserved | — | Tied to '1' |

Coinage (B2:0) fixed at `"111"`. Bits A6, A2 (unused) tied to `'1'`.

## 8. Patches

Two modifications to the upstream tree are tracked and applied during `make setup`:

- **`contrib/code/scandoubler_fix.patch`** — Vivado 2020.2 Verilog parser fix.
  The pristine `mist/scandoubler.v` contains the offending unpacked-array size
  declaration `sd_buffer[2*2**HCNT_WIDTH]` (line 124) that Vivado rejects
  (Synth 8-2671). Applied in-place to `mist/scandoubler.v`.

- **`contrib/code/galaga_dipswitch.patch`** — promotes dip-switch constants to
  entity-level input ports on `rtl_dar/galaga.vhd`. Adds `dip_switch_a` and
  `dip_switch_b` as `in std_logic_vector(7 downto 0)` to the entity port list;
  removes the internal signal declarations and hard-coded assignments. The
  Basys3 wrapper drives these ports from `sw(12 downto 0)` (§7.3).

Both patches use `patch -p1 --forward` (idempotent — skips already-applied hunks).

Generated/top-level patches:

- `contrib/basys3/code/galaga_de10_lite_to_basys3.patch` — auto-generated by
  `make_de10_lite_to_basys3_patch.sh`; diffs `galaga_de10_lite.vhd` (stale
  upstream top, kept only as diff base) against the authored
  `arcade_galaga_basys3.vhd` wrapper. Applied to the checked-in copy during
  `make patch` (not `make setup`).
...
```vhdl
vga_r <= ... vga_r_o(5 downto 2) ...;   -- VGA mode branch of the fn_toggle(7) mux
...
clock_36 --> [mod-6] --> clock_12 -------------------------------------> clk_sys
                        ...
                        '1' --------------------------------------------> ce_x2
                        "00" -------------------------------------------> scanlines
hs_out --> hsync_o --(fn_toggle(7)=0)--> vga_hs
                                                                           vs_out --> vsync_o --(fn_toggle(7)=0)--> vga_vs
                                                                           r/g/b_out(5:2) -(fn_toggle(7)=0)-----> vga_r/g/b --> VGA conn.
```

### 12.6 Signals not connected to the scan doubler

`video_csync` bypasses the doubler and feeds the `fn_toggle(7)='1'` TV branch only
