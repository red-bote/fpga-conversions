#!/bin/bash
# Generate the clk_wiz_0 MMCM IP (100 MHz -> 40.32 MHz core) for the
# Basys3 port and place its Verilog wrappers where popeye_basys3.xpr expects them.
#
# The main project's .xpr references two imported files (popeye_basys3.xpr):
...
#
# Popeye is native-progressive (no external scandoubler), so a single 40.32 MHz
# output suffices. Solved MMCM (per machine README): DIVCLK_DIVIDE=5,
...
XPR_DIR="$(cd "$(dirname "$0")/../../../vhdl_popeye_rev_0_3_2020_01_27/basys3" && pwd)"
CLK_WIZ_IMPORT_DIR="$XPR_DIR/popeye_basys3.srcs/sources_1/imports/clk_wiz_0"
