#!/bin/bash
# Create the initial Vivado Basys3 project for the Popeye port in the
...
# the project sources tree is basys3/popeye_basys3.srcs/.
#
...
CONSTRS_IMPORT="$PROJ_DIR/popeye_basys3.srcs/constrs_1/imports/digilent-xdc-master"

if [ ! -d "$SRC_DIR" ]; then
    echo "error: source tree not found: $SRC_DIR" >&2
    echo "Run contrib/tools/setup_popeye.sh first." >&2
    exit 1
fi
...
step "2/3 Copying popeye_basys3.xpr"
cp -f "$CONTRIB/vivado/popeye_basys3.xpr" "$PROJ_DIR/popeye_basys3.xpr"
...
ls -l "$PROJ_DIR/popeye_basys3.xpr"
